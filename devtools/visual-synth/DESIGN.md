...
Bu belge `devtools/visual-synth/src/` (prosedürel raster sentezi) ve onun tüketici
...
Kardeş belge: [`sound-synth.md`](../audio-synth/DESIGN.md). Görsel sentez, ses
...
`devtools/visual-synth/src/` **DOM tanımaz**. `Canvas`, `ImageData`, `window` geçmez.
...
yayınlanır. Ses tarafındaki `@volstudio/audio-synth/writer` deseninin
...
## 9. Ölçüm — `src/qa.ts` + `scripts/visual-synth-qa.ts`
