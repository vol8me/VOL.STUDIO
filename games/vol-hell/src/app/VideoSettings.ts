...
function mergeWithDefaults(stored: unknown): VideoSettingsData {
  ...
  return {
    ...
    resolution,
    graphicsQuality: isGraphicsQuality(raw.graphicsQuality)
      ? raw.graphicsQuality
...
export class VideoSettings {
  ...
  getMaxDpr(): number {
    return videoConfig.quality[this.data.graphicsQuality].maxDpr;
  }

  getParticleScale(): number {
    return videoConfig.quality[this.data.graphicsQuality].particleScale;
  }
  ...
  setGraphicsQuality(graphicsQuality: GraphicsQuality): Promise<void> {
    if (!isGraphicsQuality(graphicsQuality)) return Promise.resolve();
