...
describe('RigMotionModel', () => {
  ...
  it('sabit yön niyetinde intentFacingRad o açıya yakınsar', () => {
    const model = new RigMotionModel();
    ...
    expect(signals.intentFacingRad).toBeCloseTo(0, 1);
    expect(signals.motion01).toBeCloseTo(1, 5);
  });

  it('sürdürülen hareket sonunda intentTurnRateRadPerSec sıfıra söner', () => {
    const model = new RigMotionModel();
    ...
    expect(Math.abs(signals.intentTurnRateRadPerSec)).toBeLessThan(0.05);
  });

  it('ani 180° yön değişiminde intentTurnRateRadPerSec sıçrar sonra söner', () => {
    const model = new RigMotionModel();
    for (let i = 0; i < 60; i++) model.update(new Vector2(1, 0), DT);

    const justAfterFlip = model.update(new Vector2(-1, 0), DT);
    expect(Math.abs(justAfterFlip.intentTurnRateRadPerSec)).toBeGreaterThan(0.1);

    let settled = justAfterFlip;
    for (let i = 0; i < 200; i++) settled = model.update(new Vector2(-1, 0), DT);
    expect(Math.abs(settled.intentTurnRateRadPerSec)).toBeLessThan(0.05);
  });

  it('dead-zone altındaki niyette niyet yönü donar, sıfıra sıçramaz', () => {
    const model = new RigMotionModel();
    ...
    expect(idle.intentFacingRad).toBeCloseTo(moving.intentFacingRad, 5);
  });
});
