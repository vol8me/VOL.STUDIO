 ...
 *
 *   tsx core/scripts/visual-qa.ts <dosya-ya-da-dizin>… [--json]
 */

import { readFileSync, readdirSync, statSync } from 'node:fs';
import { join, relative, resolve } from 'node:path';
import { renderSprite } from '../src/visual/index';
import { formatQaReport, measureSprite, type QaReport } from '../src/visual/qa';
...
if (targets.length === 0) {
  console.error('Kullanım: tsx core/scripts/visual-qa.ts <dosya-ya-da-dizin>… [--json]');
  process.exit(1);
}
