...
| ------ | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D17    | P2     | Sürüklerken tarayıcı aynı jesti metin seçimi olarak da yorumluyordu: kart mavi vurguya boyanıyor, imleç I-beam'e dönüyor, mobilde "kopyala" balonu açılıyordu. Koruma yalnız `.vol-ui-root` üzerindeydi — o, OYUNLARIN tam ekran kabuğu. Bileşen başka bir yere monte edildiğinde (showcase, araç paneli, herhangi bir tüketici) korumasız kalıyordu; yani bileşenin davranışı NEREYE ASILDIĞINA bağlıydı. Ölçüldü: 21 sürükleme yüzeyinin **19'u** korumasızdı ve bir Kanban kartını sürüklemek 36 karakter seçiyordu. | Koruma sürükleme yüzeylerinin kendisine taşındı (`base.css`, gerekçesiyle). Seçim 36 → **0 karakter**. İki regresyon testi eklendi: gerçek fare sürüklemesiyle seçim ölçümü, ve her yüzeyin `user-select`ini KENDİ kapattığının doğrulanması. Liste bilinçli olarak dar: metin seçimini genel kapatmak bir açıklamayı kopyalamayı engellerdi. |
| D18    | P2     | Kart ızgarası son satırda ölü hücre bırakıyordu. `repeat(auto-fit, minmax(260px, 1fr))` boş SÜTUNLARI toplar ama boş HÜCRELERİ toplamaz: on üç kart üç sütuna dizilince son satırda bir kart ve iki ölü hücre kalır. Ölçüldü — beş sekmede toplam **on yetim hücre** (text 2, panels 3, hud 2, workbench 1, scroll 2).                                                                                                                                                                                                  | Yerleşim flex'e alındı; son satırdaki kartlar kalan genişliği paylaşıyor. Yetim hücre **10 → 0**, on iki sekmenin hepsinde.                                                                                                                                                                                                                   |
| D19    | P3     | D18'in yan etkisi: `span` modifikasyonları `grid-column` ile yazılmıştı ve flex'e geçişte SESSİZCE işlevsiz kaldı — kural duruyor, hiçbir şey yapmıyordu.                                                                                                                                                                                                                                                                                                                                                               | Dördü de `flex-basis` karşılığına çevrildi; yüzdeler `gap` payını bırakacak şekilde hesaplandı. Ekran görüntüsüyle doğrulandı: `forms` sekmesinde geniş kartlar (TIMER BAR, RANGE SLIDER, TOOLBAR) satırı doğru dolduruyor.                                                                                                                   |

**İncelendi, hata DEĞİL.** Üç şikâyet ölçümle çürüdü:

- _"Kanban'da 3'ten fazla sürükleyemiyorum"_ — demonun kendi `wipLimit: 2`
  ayarı. Ekran görüntüsüyle doğrulandı: sütun başlığında **"1 / 2"** sayacı
  görünüyor, hedef sütun kabul ederken turuncu, reddederken kırmızı vurgulanıyor.
  Ardışık dört sürükleme sınandı: birincisi geçti, üçü reddedildi.
- _"Sürüklemede bazen buglar oluyor"_ — 12 kaotik sürükleme (hızlı, düzensiz,
  yön değiştiren) sonrası sıfır yetim hayalet, sıfır takılı sınıf, sıfır konsol
  hatası, kart sayısı sabit.
- _"Kartlar kesişiyor / içine sıkışıyor"_ — ölçüldü: 964 px ızgarada üç kart
  300 + iki boşluk 32 = tam 964. Slot grid 495 px, 561 px kartın içinde.
  Görsel "kayma" sandığım şey 2×2 kaplayan eşya, yani demonun gösterdiği özellik.
